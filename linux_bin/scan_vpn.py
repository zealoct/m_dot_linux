...
for k in range(23, 24):
    ...
    for i in range(232, 234):
        for j in range(0, 256):
            cmd="sshpass -p {} " \
                "ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null {}@172.{}.{}.{} \"hostname\"".format(passwd, username, k, i, j)
        ...
        for j in range(0, 256):
            ...
            if ps[j].returncode == 0:
                hostname, _ = ps[j].communicate()
                print "{} : 172.{}.{}.{}".format(hostname.strip(), k, i, j)
